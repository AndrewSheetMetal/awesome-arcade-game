...
{	
	...
	// Gibt an, ob bereits auf Kollision f�r diesen Zyklus �berpr�ft wurde. 
	...
	// Pr�fen, ob ein Gegner gegen eine Wand st��t und ggf. seine Richtung �ndern.
	public void handleIntersectionWithWall(List<Battlefield> pBattlefields, double speed)
	{
		for(Battlefield lBf : pBattlefields)
		{
			// Gegner befindet sich im aktuellen Spielfeld und ber�hrt eine Wand.
			if(hitsBattlefieldBorder(lBf))
			{
				// 0: 0 = waagerecht, 1 = senkrecht; 1 : Anzahl Pixel innerhalb
				double[] lNearest = {0, 0};
				// Falls der Gegner �ber eine Wand hinausragt, wird hier angegeben, wie weit er hineinragt.
				double lDepth = 0;
				// Der Index des zweiten Punktes einer Wand.
				int lSecond = 0;
				// Gibt an, ob der Gegner �ber einen Teil der Mauer hinausragt.
				...
				for(int i = 0; i < lBf.npoints; i++)
				{
					...
					if(intersectsWall(lWall))
					{
						...
						{
							// Ragt der Gegner oben etwas �ber die Mauer hinaus?
							...
							// Ragt der Gegner unten etwas �ber die Mauer hinaus?
						...
						{
							// Ragt der Gegner links etwas �ber die Mauer hinaus?
							...
							// Ragt der Gegner rechts etwas �ber die Mauer hinaus?
	...
	// ALEX: setDirection gel�scht und in Player und Enemy eingef�gt.	
	...
	// Liefert true, wenn der Gegner den Rand eines Schlachtfeldes ber�hrt.
	...
	// Liefert true, wenn der Gegner das Schlachtfeld ber�hrt.
	...
	// Liefert true, wenn der Gegner die Linie ber�hrt.
	...
	// �ndert die Richtung des Gegners abh�ngig von der Art der Wand. (0 = horizontal; 1 = vertikal)
	...
	// Erzeugt eine zuf�llig gew�hlte g�ltitge Position. 
	// TODO: Ohne zweiten Parameter l�sen.
	...
	// Liefert einen zuf�llig erzeugten Spawnpoint.
	...
	// Liefert eine zuf�llige Koordinate innerhalb des Spielfeldes.
	...
	// Liefert ggf. einen Gegner, der vom aktuellen Gegner ber�hrt wird.
	...
	public void handleIntersectionWithFriends()
	{
		...
		if((lEnemy != null) && (!this.getHandled() || !lEnemy.getHandled()))
		{
			...
			// Anderen Gegner auf gegen�berliegenden Winkel setzen.
			lAngle += 180;
	...
	// Wandelt einen �bergebenen Winkel in einen Punkt-Richtung um.
